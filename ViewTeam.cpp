...
void ViewTeam::populateData(td::INT4 type) {
	...
	_pDS1 = _db1->createDataSet("SELECT T.ID as ID, T.Ime as Tim, T.Opis as Opis,  T.BrClanova as 'Broj ljudi', CAST(12.53 AS DECIMAL(7,2) ) as 'Brzina', COUNT(CASE WHEN Ti.stanje = 0 THEN 1 END) AS 'Dodani Tiketi',COUNT(CASE WHEN Ti.stanje = 1 THEN 1 END) AS 'Tiketi u progresu',COUNT(CASE WHEN Ti.stanje = 2 THEN 1 END) AS 'Zavrseni tiketi' FROM Tim AS T LEFT JOIN Projekti AS P ON T.ProjekatID = P.ID LEFT JOIN Tiketi AS Ti ON P.ID = Ti.ProjekatID Where t.ID!=-1 GROUP BY T.ID, T.BrCLanova");
	dp::DSColumns cols(_pDS1->allocBindColumns(8));
	td::String TimName, Opis;
	td::INT4 Brcl, Dodan, Progres, Zavrsen, ID;
	double Brzina;
	cols <<"ID"<<ID << "Tim" << TimName <<"Opis" <<Opis<< "Broj ljudi" << Brcl <<"Brzina"<<Brzina << "Dodani Tiketi" << Dodan << "Tiketi u progresu" << Progres << "Zavrseni tiketi" << Zavrsen;
