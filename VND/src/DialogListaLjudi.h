...
protected:
	...
	gui::TableEdit* _te;
public:
	DialogListaLjudi(gui::Frame* pView, gui::TableEdit* _te1) :
		gui::Dialog(pView,
			{ { gui::Dialog::Button::ID::Cancel, tr("Ok"),gui::Button::Type::Constructive },
			{ gui::Dialog::Button::ID::Apply, tr("Cancel") },
			{ gui::Dialog::Button::ID::OK, tr("Ponisti"),gui::Button::Type::Destructive },
			}, gui::Size(500, 400)
		)
		, _te(_te1)
	{
	...
	bool onClick(gui::Dialog::Button::ID btnID, gui::Button* pButton) override
	{
		if (btnID == gui::Dialog::Button::ID::OK) {
			...
			//Ovo ce biti za Ponisti
			_viewDialogListaLjudi.ponisti();
			return false;
		}
		else if (btnID == gui::Dialog::Button::ID::Cancel) {
			//Ovo ce biti za OK
			if(!_viewDialogListaLjudi.spasi())return false;
			...
			for (size_t i = 0; i < _sviRedovi.size(); ++i)
			{
				...
				if (_sviRedovi.size()==1 && i == _sviRedovi.size() - 1) {
					row[4] = 0;
					_te->insertRow(i);
					_te->endUpdate();
					break;
		...
		else if (btnID == gui::Dialog::Button::ID::Apply) {
			//Ovo ce biti za Izadji
			return true;
		}
		return true;
	}
